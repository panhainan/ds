...
import com.ds.dreamstation.util.MD5;
...
public class MemberController {
  ...
  public String teacher(@RequestParam(required = false, defaultValue = "1") int current, HttpServletRequest req) {
    int size = 12;
    // 导师的类别为3
    int category = 3;
    Map<String, Object> map = this.memberService.paging(current, size, category);
    List<MemberInfo> listTeacherInfo = (ArrayList<MemberInfo>) map.get("listMemberInfo");
  ...
  public String programmer(@RequestParam(required = false, defaultValue = "1") int current, HttpServletRequest req) {
    int size = 12;
    // 程序员的类别为1
    int category = 1;
    Map<String, Object> map = this.memberService.paging(current, size, category);
    List<MemberInfo> listProgrammerInfo = (ArrayList<MemberInfo>) map.get("listMemberInfo");
  ...
  public String designer(@RequestParam(required = false, defaultValue = "1") int current, HttpServletRequest req) {
    int size = 12;
    // 设计师的类别为2
    int category = 2;
    Map<String, Object> map = this.memberService.paging(current, size, category);
    List<MemberInfo> listDesignerInfo = (ArrayList<MemberInfo>) map.get("listMemberInfo");
  ...
  public @ResponseBody
  String get_verification_code(HttpServletRequest request) {
    ...
    request.getSession().setAttribute("verification_code", MD5.get_MD5(str.toString()));
    return str.toString();
  }
  ...
  public String login(String loginEmail, String loginPass, String verification_code, HttpServletRequest req) {
    Member member = this.memberService.login(loginEmail, loginPass);
    HttpSession session = req.getSession();
    if (null == member) {
      return "redirect:/manage/login-failed";
    } else if (!(MD5.get_MD5(verification_code)).equals(session.getAttribute("verification_code"))) {
      return "redirect:/manage/login-failed-verification";
    }
    else {
      MemberSession memberSession =
          new MemberSession(member.getmId(), member.getmLoginEmail(), member.getmName(), member.getmUserAuthority());
  ...
  public String editPic(HttpServletRequest request, @RequestParam("fileUpload") MultipartFile fileUpload) {
    // 调用工具类FileUpload的静态方法来进行图书上传
    StringBuffer picSite = FileUpload.upload(request, fileUpload, "member", true);
    ...
    this.memberService.editPicture(memberSession.getId(), picSite.toString());
    return "redirect:/member/self/selfInfo";
  }
  ...
  public String editPass(String oldPass, String newPass, String repeatNewPass, HttpServletRequest req) {
    ...
    if (!newPass.equals(repeatNewPass)) {
      ...
      HttpSession session = req.getSession();
      MemberSession memberSession = (MemberSession) session.getAttribute("memberSession");
      message.append(this.memberService.editPass(memberSession.getId(), oldPass, newPass));
    }
  ...
  public String list(@RequestParam(required = false, defaultValue = "1") int current, HttpServletRequest req) {
    ...
    req.setAttribute("listMemberInfo", (ArrayList<MemberInfo>) map.get("listMemberInfo"));
    req.setAttribute("pageBean", (Pages) map.get("pageBean"));
  ...
  public String authoriting(AuthorityForm authorityForm, HttpServletRequest request) {
  ...
  public String remove(int id, HttpServletRequest request) {
    ...
    if (memberSession.isManageMember() || memberSession.isManageAdmin()) {
      Member member = this.memberService.find(id);
      if (!member.getmUserAuthority().isaManageMember() && !member.getmUserAuthority().isaManageAdmin()) {
        String basePath =
            request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()
                + request.getContextPath() + "/";
        MailType mailType = new MailType();
