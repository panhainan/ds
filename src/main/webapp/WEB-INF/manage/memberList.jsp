	...
	<div class="rightinfo">
		...
		<c:choose>
			<c:when test="${listMemberInfo!=null}">
						...
						<c:forEach items="${listMemberInfo}" var="memberInfo">
							<li><span><img width="160px"
								...
								<div class="lright">
									...
									<a class="enter"
										href="<%=basePath%>member/manage/look?id=${memberInfo.id}"
										style="width: 80px;float: left;">查看详情</a>
									<c:if test="${memberSession.manageMember==true}">
										<a class="enter"
											onclick="return checkRemove('${memberInfo.id}');"
											style="width: 40px;float: left;margin-left: 15px;">删除</a>
									</c:if>
								</div>
							</li>
						</c:forEach>
				...
				<div class="tip">
					<form action="<%=basePath%>member/manage/remove" method="post">
						<input type="hidden" name="id" id="removeMemberId">
						<div class="tiptop">
							<span>提示信息</span><a></a>
						</div>
