...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$(".removeBtn").click(function() {
			var checkBoxs = document.getElementsByName("newsInfoCheckBox");
			var j=0;
		...
		$(".sure").click(function() {
			...
			$(".tip").fadeOut(100);
			window.location.href="<%=basePath%>news/manage/removes?ids="+ _ids;
		});
		...
		$(".auditingtipsure").click(function() {
			...
			for ( var i = 0; i < statusAuditings.length; i++) {
				if(statusAuditings[i].checked == true){
					...
					window.location.href="<%=basePath%>news/manage/auditing?id="+$('.auditingId').html()+"&status="+statusAuditings[i].value;
				};
			};
		});
	...
	function pitchOn(obj) {
		var s = obj.id;
		var clickRowCheckBox = "newsInfoCheckBox+"
	...
	function changeStatus(obj){
		window.location.href="<%=basePath%>news/manage/list?status="+obj.value;
	}
	function auditing(id,title){
		$(".auditingTitle").html("“"+title+"”");
		$(".auditingId").html(id);
...
<body>
	...
	<div class="place">
		<span>位置：</span>
		<ul class="placeul">
			<li><a href="<%=basePath%>news/manage/list?current=1">新闻管理</a>
			</li>
		</ul>
	</div>
	<div class="rightinfo">
		...
		<div class="tools">
			...
			<ul class="toolbar">
				<li><a href="<%=basePath%>news/manage/go-add"><span><img
			...
			<c:when test="${listNewsInfo!=null}">
					...
					<table class="tablelist">
						...
						<tbody>
							...
							<c:forEach items="${listNewsInfo}" var="newsInfo">
								<tr id="newsInfoTr+${newsInfo.id}" onclick="pitchOn(this)">
									<td><input name="newsInfoCheckBox"
										id="newsInfoCheckBox+${newsInfo.id}" type="checkbox"
										value="${newsInfo.id}" /></td>
									<td><%=i++%></td>
									<td>${newsInfo.title}</td>
									<td>${newsInfo.author}</td>
									<td>${newsInfo.publishTime}</td>
									<td>
									
									${newsInfo.status}(<label onclick="auditing('${newsInfo.id}','${newsInfo.title}')" style="cursor: pointer;"><img style="width: 14px;" src="<%=basePath%>images/t04.png" />审核</label> )
									
									</td>
									<td><a
										href="<%=basePath%>news/manage/look?id=${newsInfo.id}"
										class="tablelink"><img style="width: 14px;"
											src="images/ico06.png" />查看</a> <a
										href="<%=basePath%>news/manage/go-edit?id=${newsInfo.id}"
										class="tablelink"><img style="width: 14px;"
											src="images/t02.png" />编辑</a> <a
										href="<%=basePath%>news/manage/remove?id=${newsInfo.id}"
										class="tablelink"><img style="width: 14px;"
					...
					<ul class="paginList">
							...
							<c:choose>
								<c:when test="${pageItem==pageBean.currentPage}">
									<li class="paginItem current"><a
										href="<%=basePath%>news/manage/list?current=${pageItem}">${pageItem}</a>
									</li>
								</c:when>
								<c:otherwise>
						...
						<c:choose>
							<c:when test="${pageBean.totalPages!=pageBean.currentPage}">
								<li class="paginItem"><a
									href="<%=basePath%>news/manage/list?current=${pageBean.totalPages}"><span
				...
				<div class="tip">
					...
					<div class="tipinfo">
						...
						<div class="tipright">
							<p>是否确认要删除这<label id="removeRows"></label>条新闻吗？</p>
				...
				<div class="auditingtip" style="width: 500px;">
					<div class="tiptop">
						<span>审核新闻</span>
					</div>
